...
ctypedef np.complex_ cplx_t
ctypedef np.ndarray[float, ndim=1] vect_float
ctypedef np.ndarray[float, ndim=2] matr_float
ctypedef np.ndarray[cplx_t, ndim=1] vect_cplx
ctypedef np.ndarray[cplx_t, ndim=2] matr_cplx

cdef inline float float_max(float a, float b): return a if a >= b else b
cdef inline float float_min(float a, float b): return a if a <= b else b

def hann_window(int size):
	float c0, c1, c2 = 0.5, -0.5, 0
	vect_float res = np.empty(size)
	for i in xrange(size):
		res[i] = c1 * cos(2.0 * np.pi * i / size) + c2 * cos(4.0 * np.pi * i / size)
	return res

def windowed(vect_float sample,int window_size,float overlap,bool hanning=True):
	assert 0 <= overlap < 1
	vect_matr res = np.empty((window_size,
	vect_float win = np.hanning(window_size)
...
def noise_reduce(sample,noise,settings):
	...
	for center in xrange(WINDOW_COUNT):
		if SPECTRUM_MEDIAN_WINDOW > 1:
			i0 = float_max(center - (SPECTRUM_MEDIAN_WINDOW / 2),0)
			i1 = float_min(center + (SPECTRUM_MEDIAN_WINDOW / 2) + 1,WINDOW_COUNT - 1)
...
def smooth(sample,width=3,geom=True):
	...
	for i in xrange(len(sample)):
		i0 = float_max(0,i - width / 2)
		i1 = float_min(len(sample) - 1,1 + i + width / 2)
