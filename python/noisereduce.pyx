...
def hann_window(int size):
	...
	cdef float c1 = -0.5
	cdef float c2 = 0.0
	cdef np.ndarray[float, ndim=1] res = np.empty(size)
	cdef int i
	for i in xrange(size):
		res[i] = c0 + c1 * cos(2.0 * np.pi * i / size) + c2 * cos(4.0 * np.pi * i / size)
	return res

def windowed(np.ndarray[float, ndim=1] sample,int window_size,float overlap,bool hanning=True):
	assert 0 <= overlap < 1
	cdef int num_windows = count_windows(len(sample),window_size,overlap)
	cdef np.ndarray[float, ndim=2] res = np.empty((num_windows,window_size))
	cdef np.ndarray[float, ndim=1] win = np.hanning(window_size)
	cdef int i, start
	cdef np.ndarray[float, ndim=1] sub
...
def count_windows(int sample_length,int window_size,float overlap):
...
def noise_reduce(sample,noise,settings):
	...
	# Input constants
	cdef float NOISE_GAIN = settings.noise_gain # Gain (negative) for noise
	cdef float SENSITIVITY = settings.sensitivity # How much louder must a band be than avg. noise to pass?
	...
	cdef float ATTACK_TIME = settings.attack_time # Attack time
	cdef float RELEASE_TIME = settings.release_time # Release time
	...
	cdef float OVERLAP = float(WINDOW_SIZE - WINDOW_STEP) / WINDOW_SIZE # What fraction of window (i) overlaps with window (i+1)?
	...
	cdef float ATTACK_CONST = from_db(NOISE_GAIN / N_ATTACK_BLOCKS) # Attack decay constant
	cdef float RELEASE_CONST = from_db(NOISE_GAIN / N_RELEASE_BLOCKS) # Release decay constant
	cdef float NOISE_ATTEN_FACTOR = from_db(NOISE_GAIN) # Minimum attenuation factor
	...
	# Prepare data arrays
	cdef np.ndarray[float, ndim=1] noise_f_db = noise_mean(noise,WINDOW_SIZE,OVERLAP,DOUBLE_WINDOW)
	cdef np.ndarray[float, ndim=1] window = hann_window(WINDOW_SIZE)
	cdef np.ndarray[float, ndim=2] gains = np.full((WINDOW_COUNT,WINDOW_SIZE),NOISE_ATTEN_FACTOR)
	...
	cdef np.ndarray[float, ndim=1] frame, spectrum_med_db
	...
	cdef np.ndarray[float, ndim=1] result = np.empty(len(sample) + WINDOW_SIZE)
...
def classify_gains(np.ndarray[float, ndim=1] gains,np.ndarray[float, ndim=1] sample_db,np.ndarray[float, ndim=1] noise_db,float sensitivity,float gain_factor):
	cdef int i
	for i in xrange(len(gains)):
		gains[i] = 1 if sample_db[i] > noise_db[i] + sensitivity else gain_factor

def noise_mean(np.ndarray[float, ndim=1] noise,int window_size,float overlap,bool double=False):
	cdef int frames = 1 + int(len(noise) / (window_size * (1 - overlap)))
	cdef int i
	cdef np.ndarray[float, ndim=1] means = np.empty((frames,window_size))
	cdef np.ndarray[float, ndim=1] window = hann_window(window_size)
...
def attack(np.ndarray[float, ndim=2] gains,int center,spectrum,lookback,float attack_const,float min_gain):
	cdef int freq
	cdef int l
	cdef float minimum
...
def release(np.ndarray[float, ndim=2] gains,int center,spectrum,float release_const,float min_gain):
	cdef int freq
	for freq in spectrum:
		if center > 0:
			gains[center][freq] = float_max(
				gains[center][freq],
				float_max(
...
def smooth(np.ndarray[float, ndim=1] sample,int width=3,bool geom=True):
	cdef np.ndarray[float, ndim=1] ref = np.copy(sample)
	cdef int i, i0, i1
	cdef float avg
	if geom:
		ref = to_db(ref)
