...
def to_db(val):
	return 10 * np.log10(0.0000001 + (np.abs(val)) ** 2)
...
def noise_reduce(sample,noise,settings):
	...
	cdef int N_ATTACK_BLOCKS = 1 + int(ATTACK_TIME * RATE / WINDOW_STEP) # Attack blocks
	cdef int N_RELEASE_BLOCKS = 1 + int(RELEASE_TIME * RATE / WINDOW_STEP) # Release blocks
	...
	cdef np.ndarray[double, ndim=1] result = np.zeros(len(sample) + WINDOW_SIZE)
	i = 0
	...
	return float(WINDOW_STEP) / WINDOW_SIZE * result
